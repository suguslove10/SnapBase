...
export default function BillingPage() {
  ...
  const [soc2From, setSoc2From] = useState("2026-01-01");
  const [soc2To, setSoc2To] = useState(new Date().toISOString().split("T")[0]);
  const [exportingSOC2, setExportingSOC2] = useState(false);

  const handleStripeCheckout = async (plan: string) => {
    try {
      const res = await api.post("/billing/stripe/checkout", { plan });
      if (res.data.url) {
  ...
  const handleDownloadSOC2 = async (e: React.FormEvent) => {
    e.preventDefault();
    setExportingSOC2(true);
    try {
      const res = await api.post(
        "/reports/compliance",
        { from: soc2From, to: soc2To },
        { responseType: "blob" }
      );
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `SnapBase-SOC2-Audit-Package-${soc2From}-to-${soc2To}.pdf`);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-grotesk text-2xl font-bold text-white">Billing & Compliance</h1>
          <p className="mt-1 text-sm text-slate-500">Manage your subscription, Stripe portal, and SOC2 audit exports.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <CreditCard className="h-4 w-4 text-[#00b4ff]" />
            Stripe Portal
          </button>
          <button
            onClick={() => setShowSOC2Modal(true)}
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#00b4ff] to-[#0077ff] px-4 py-2.5 text-xs font-semibold text-white shadow-lg shadow-[#00b4ff]/20 transition hover:opacity-90"
          >
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/10 p-6 shadow-2xl" style={{ background: "#0c1322" }}>
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <h3 className="font-grotesk text-lg font-bold text-white flex items-center gap-2">
                <Receipt className="h-5 w-5 text-[#00b4ff]" />
                Export SOC2 Audit Package
              </h3>
              <button onClick={() => setShowSOC2Modal(false)} className="text-slate-400 hover:text-white">✕</button>
            </div>

            <form onSubmit={handleDownloadSOC2} className="mt-4 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">From Date</label>
                <input
                  type="date"
                  value={soc2From}
                  onChange={(e) => setSoc2From(e.target.value)}
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white focus:border-[#00b4ff] focus:outline-none"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="date"
                  value={soc2To}
                  onChange={(e) => setSoc2To(e.target.value)}
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white focus:border-[#00b4ff] focus:outline-none"
                  required
                />
              </div>

              <div className="rounded-xl border border-[#00b4ff]/20 bg-[#00b4ff]/5 p-3 text-xs text-slate-400">
                Includes AES-256-GCM proofs, S3 WORM Object Lock verification logs, and SHA-256 audit signature.
              </div>

              <div className="pt-2 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setShowSOC2Modal(false)}
                ...
                >
                  {exportingSOC2 ? "Generating PDF..." : "Download PDF Report"}
                </button>
              </div>
            </form>
          </div>
